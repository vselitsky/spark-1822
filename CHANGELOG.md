...
- `tailscale/services.json`: new file. Tailscale [VIP Service](https://tailscale.com/kb/1417/services) endpoint config for `svc:spark` — a separate tailnet entity with its own MagicDNS name (`spark.<tailnet>.ts.net`) that hosts on this node. Endpoints mirror the node-level setup (`tcp:80` → `http://traefik:80`, `tcp:443` → `https-insecure://traefik:443`). Distinct schema from `serve.json` (per [Tailscale Services configuration file](https://tailscale.com/kb/1589/tailscale-services-configuration-file)) — loaded into the daemon via `tailscale serve set-config --service=svc:spark <file>`, advertised via `tailscale serve advertise svc:spark`. Both are tailnet-specific state and live in the `tailscale-state` docker volume rather than the committed compose. `tailscale/README.md` documents the two-step apply.
